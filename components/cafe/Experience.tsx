...
export default function Experience() {
  // Use Variants type and 'as const' to force the compiler to accept 'spring'
  ...
  const textVariants: Variants = {
    ...
    show: {
      ...
      transition: { 
        ...
        stiffness: 100, 
        damping: 20 
      } as any, // The "as any" is a temporary emergency bypass for your demo
    },
  };
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .experience-subtitle { font-family: 'Nunito', sans-serif; font-weight: 700; color: rgba(255, 255, 255, 0.9); line-height: 1.6; }
        .experience-card { border-radius: 40px; padding: 36px 28px; text-align: center; box-shadow: 0 20px 50px -15px rgba(0,0,0,0.15); cursor: pointer; border: 4px solid white; transition: all 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275); }
        .experience-card:hover { transform: translateY(-12px); box-shadow: 0 30px 60px -15px rgba(0,0,0,0.25); }
      `}</style>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative z-10 flex flex-col items-center">
                    <motion.div 
                      animate={{ y: [0, -6, 0] }}
                      transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: index * 0.2 } as any}
                    ...
                    <h3 className={`text-[24px] font-black mb-3 ${item.titleColor}`} style={{ fontFamily: "'Baloo 2', cursive" }}>{item.title}</h3>
                    <p className="text-slate-600/90 font-bold text-[15px] leading-relaxed" style={{ fontFamily: "'Nunito', sans-serif" }}>{item.desc}</p>
                  </div>
                </motion.div>
              );
            })}
